...
def read_csv_from_s3():
    try:
        ...
        df['icustay_id'] = df['icustay_id'].fillna(value=2222).round(decimals=3).replace(' ', '').replace(np.nan, 2222)
         # Reemplazar los valores NaN, nulos y vacíos en la columna "value" con el valor 2222 en una sola línea de código
        df['value'] = df['value'].fillna(value=2222).replace(' ', '').replace(np.nan, 2222)
        # Reemplazar los valores NaN, nulos y vacíos en la columna "valuenum" con el valor 2222 en una sola línea de código
        df['valuenum'] = df['valuenum'].fillna(value=2222).replace(' ', '').replace(np.nan, 2222)
        # Reemplazar los valores NaN, nulos y vacíos en la columna "warning" con el valor 2222 en una sola línea de código
        df['warning'] = df['warning'].fillna(value=2222).replace(' ', '').replace(np.nan, 2222)
        # Reemplazar los valores NaN, nulos y vacíos en la columna "error" con el valor 2222 en una sola línea de código
